...
import bulldog.eshop.ws.products.*;
...
public class ProductsEndpoint {
	...
	public GetProductByIdResponse getProductById(@RequestPayload GetProductByIdRequest request) {
		...
		Optional<ProductWS> productOptional = productService.getById(request.getId()).map(ProductService.PRODUCT_TO_WS_FACTORY);
	...
	public GetAllProductsResponse getAllProducts(@RequestPayload GetAllProductsRequest request) {
		...
		List<ProductWS> productsWS = productService.getAll().stream().map(ProductService.PRODUCT_TO_WS_FACTORY).collect(Collectors.toList());
